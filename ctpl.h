...
namespace ctpl {
	...
	public:
		...
		thread_pool(size_t nThreads, size_t queueSize = _ctplThreadPoolLength_) : q_(queueSize) { init(); resize(nThreads); }
	...
	private:
		...
		mutable boost::lockfree::queue<std::function<void(size_t id)> *> q_;
